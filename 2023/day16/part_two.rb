...
Benchmark.bmbm do |x|
  x.report('Day 16 - Part 2') do
    ...
    def move_to(x, y, direction)
      return if x.negative? || y.negative? || x >= @rows || y >= @cols # can't go outside
    ...
    # left & right
    projections = @rows.times.flat_map { [[_1, 0, 'R'], [_1, @cols - 1, 'L']] }
    # top  bottom
    projections += @cols.times.flat_map { [[0, _1, 'D'], [@rows - 1, _1, 'U']] }
    result = projections.each_with_object({}) do |projection, hash|
      @travel_logs = Hash.new { |h, k| h[k] = Hash.new(0) }
      move_to(*projection)
      key = "#{projection[0]}/#{projection[1]}:#{projection[2]}"
      hash[key] = @travel_logs.size
    end
    result.values.max
  end
end
