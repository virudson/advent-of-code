...
labeled "[Hot Springs](https://en.wikipedia.org/wiki/Hot_spring)" pointing in a
...
contains the animal is  _one large, continuous loop_.
...
need to find the tile that would take the longest number of steps  _along the
loop_  to reach from the starting point - regardless of which way around the
...
Maybe its nest is  _within the area enclosed by the loop_?
...
marked  `O`  below) are  _not_  in the loop. Here is the same loop again with
...
within the loop.  _How many tiles are enclosed by the loop?_
