...
just installed a new  _server rack_, but we aren't having any luck getting the
...
through the devices. Data only ever flows from a device through its outputs; it
...
and  `fft`  (a device which performs
...
you to find every path from  `svr`  (the server rack) to  `out`. However, the
paths you find must all also visit both  `dac`  _and_  `fft`  (in any order).
...
However, only  _`2`_  paths from  `svr`  to  `out`  visit both  `dac`  and
`fft`.

Find all of the paths that lead from  `svr`  to  `out`.  _How many of those
paths visit both  `dac`  and  `fft`?_
